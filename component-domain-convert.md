...
- ✅ 领域模型转换、集合结构构建、复杂参数组装、复杂查询条件组装必须在 Convert 中实现，Service 仅负责流程编排与调用
- ✅ 当查询参数小于等于 2 个且均为 `eq` 等值条件时，可在 Service 直接构建等值查询，不强制封装 QueryDTO
...
- ✅ QueryDTO 适用于多条件组合、非等值查询、可复用查询模板场景
- ✅ 小于等于 2 个参数且仅 `eq` 查询时，可不定义 QueryDTO
- ❌ 禁止在 DTO 中手动构建查询条件
...
**规则**
- ✅ 多条件组合、非等值、动态查询必须使用 `BasicService.dtoInfosToWrapper()` 或 `pageByPagerDto()`
- ✅ 小于等于 2 个参数且仅 `eq` 查询时，允许在 Service 层直接构建等值查询
...
- ✅ 集合结构构建（如 `SpecOption`、路径节点、中间聚合结构）必须放在对应 Convert 中实现并复用
- ✅ 参数组装与查询条件组装（多条件/复用场景）必须放在 Convert 中实现，Service 直接调用
- ✅ 小于等于 2 个参数且仅 `eq` 查询可在 Service 直接实现，不强制先封装 QueryDTO

**禁止行为**
- ❌ 禁止在 Service 中手写 DTO/Domain/VO 字段映射与批量 set
- ❌ 禁止在 Service 中重复构建可在 Convert 复用的集合中间结构
- ❌ 禁止在 Service 中散落拼装复杂查询 DTO / 多条件查询参数
